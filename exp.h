...
expData getExp(const std::string &equ, int opP) {
    ...
    std::string lSub = equ.substr(lastOp + 1, opP - lastOp - 1);
    if(!lSub.length()) return {lastOp + 1, opP + nextOp, lHalf, rHalf};
    lHalf = std::stold(lSub);
...
long double calcExp(long double lHalf, long double rHalf, char op) {
    switch(op) {
        case 'a': return lHalf==1 ? fabs(rHalf) : lHalf==2 ? asin(rHalf) : lHalf==3 ? acos(rHalf) : atan(rHalf);
        case 'f': return floor(rHalf);
        case 'c': return lHalf==1 ? ceil(rHalf) : cos(rHalf);
        case 's': return sin(rHalf);
        case 't': return tan(rHalf);
        case 'v': return lHalf==2 ? sqrt(rHalf) : cbrt(rHalf);
        case 'l': return log(rHalf);
        case 'e': return exp(rHalf);
