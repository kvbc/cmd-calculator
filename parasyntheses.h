...
long double calcEqu(const std::string &equ, bool beautify, bool legalCheck);
...
bool calcAllParasyntheses(std::string &equ) {
    while(equ.find('(') != std::string::npos) {
        ...
        std::size_t clBracket = equ.find(')', opBracket + 1);
        if(!legalParasyntheses(clBracket)) return false;
        std::string res = std::to_string(calcEqu(equ.substr(opBracket + 1, clBracket - opBracket - 1), false, false));
